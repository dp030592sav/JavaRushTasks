    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                // name
                 int res = String.valueOf(stock1.get("name")).compareTo(String.valueOf(stock2.get("name")));
                 if(res != 0)
                     return res;
                ...
                res = secondLocalDate.compareTo(firstLocalDate);
                if(res != 0)
                    return res;

                // profit
                double firstProfit,secondProfit;
                if(stock1.containsKey("change"))
                    firstProfit = (double)stock1.get("change");
                else
                    firstProfit = (double)stock1.get("last") - (double)stock1.get("open");
                if(stock2.containsKey("change"))
                    secondProfit = (double)stock2.get("change");
                else
                    secondProfit = (double)stock2.get("last") - (double)stock2.get("open");
                return Double.compare(secondProfit, firstProfit);
            }
        });
    }
