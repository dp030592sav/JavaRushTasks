...
public class Dog implements Pet {
     ...
     * а также ко всем полям и методам включая private того класса
     * <p>
     * <p/>
    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable {
            ...
            public String say() {
                String res = "";
                if (i < 1)
                    res = String.format("%s спит.", getName());
                else {
                    StringBuilder builder = new StringBuilder();
                    for (int j = 0; j < i; j++)
                        builder.append("а");
                    res = String.format("%s лает г%sв! %s", getName(), builder, super.formatter.format(new Date()));
                }
                return res;
            }
